...
export default function FormsAjouter() {
    ...
    const handleSubmitArticle = (event) => {
        ...
        const errors = validateArticleForm({ titre, revue, url, annee });
        if (Object.keys(errors).length > 0) {
          setErrorMessages(errors);
          return;
        }
    
        axios.post('articles', { titre, revue, url, annee })
          .then(response => {
      ...
      const handleSubmitBook = (event) => {
        ...
        const errors = validateBookForm({ titre, isbn, depot_legal, annee });
        if (Object.keys(errors).length > 0) {
          setErrorMessages(errors);
          return;
        }
    
        axios.post('livres', { titre, isbn, depot_legal, annee, issn })
          .then(response => {
      ...
      const validateArticleForm = ({ titre, revue, url, annee }) => {
        const errors = {};
      ...
      const validateBookForm = ({ titre, isbn, depot_legal, annee }) => {
        const errors = {};
        if (!titre) errors.titre = 'Veuillez entrer le titre';
